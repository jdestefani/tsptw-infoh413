...
- Implemented NumericMatrix template class to speed up accesses to the matrices
  (NumericMatrix.h)

- Implemented SA and MAX-MIN Antsystem.
...
Every experiment produces a file containing the results of each run:
...
For the SLS algorithms, in addition to the results file, another file is produced:

[TSPTW-ACO]

ACO.<instance_name>.rtd
	
[TSPTW-SA]

SA.<instance_name>.rtd

Ex: SA.n80w200.004.rtd
	ACO.n80w20.003.rtd
...
For each run of the algorithm, the program samples the solution quality at the sample instances described in the file,
an write the sampled value in the corresponding column, using a tabulation as separator.

The solution quality is computed as relative solution quality with respect to the known global optimum:

((currentBestSolution+constraintViolations*CONSTRAINT_VIOLATION_PENALTY)/known_best_solution) - 1

The CSV structure of the files allows an easy manipulation and post-processing using R.
...
The .pdf files contains respectively, the box-plots comparing the runtime of the different algorithms and the solution quality, measured in terms of PRPD.
...
The script takes as input exactly four parameters:
[INSTANCE_NAME]: String indicating the instance name (all the instances are assumed to be in the instances folder)
[RUNS] : Integer number indicating the number of runs of the algorithm
[SEEDS_FILE] : String indicating the name of the file where the generated seeds will be stored.
[MAX_RUNTIME] : Real number indicating the maximum runtime of the algorithm 
...
<instanceName>-PRPD.pdf
<instanceName>-RTD.pdf
ACO-<instanceName>-RTD.pdf
SA-<instanceName>-RTD.pdf
Update of ACO.stats (ACO) or SA.stats (SA) files.

The .pdf files contains:

-> <instanceName>-PRPD.pdf : Box-plots comparing the solution quality of the solutions generated by the two algorithms
-> <instanceName>-RTD.pdf: Plot comparing the run-time distributions of the two algorithms.
-> ACO-<instanceName>-RTD.pdf and SA-<instanceName>-RTD.pdf : Plot comparing the run-time distributions for different values of the solution quality q* for the same algorithm.

The .stats files are CSV files having the same name as the algorithm to evaluate, containing one line for each instance.
Each line is composed by:
